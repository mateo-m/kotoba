...
- **Fangame with `Game.exe`:** [Installing in a game](/v0.1.0/essential/installation) — `load "kotoba/boot.rb"` in Script Editor; edit `kotoba/boot.rb` on disk for `catalog_paths`, `available_locales`, and the adapter.
- **Git clone:** [Quick Start](/v0.1.0/essential/quick-start) — same API; paths relative to the repo root.

The **Setup** block below belongs in `kotoba/boot.rb` (or an equivalent boot script). Do not paste the whole block into Script Editor unless you are following [Bare RGSS](/v0.1.0/integration/bare-rgss) inline boot as an alternative to `boot.rb`.
