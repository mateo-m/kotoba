...
Git clone instead of a ZIP? [Quick Start](/v0.1.0/essential/quick-start). Translators: [For translators](/v0.1.0/translators/).
...
A passing smoke test only proves Kotoba loaded and read the **sample** catalog. It does not translate existing game dialog. After it works, continue to [§5 Your own translations](#5-your-own-translations) and [Pokemon Essentials](/v0.1.0/integration/pokemon-essentials) or [Essentials migration](/v0.1.0/integration/pokemon-essentials-migration) to bridge `_INTL`.
...
`{pokemon}` is filled at runtime. Translators keep that placeholder in every locale. See [Placeholders](/v0.1.0/translators/placeholders).
...
| `LoadError` / `No such file` | Files not next to `Game.exe`. See [Troubleshooting](/v0.1.0/essential/troubleshooting) |
...
Release ZIPs already call `Kotoba.use_adapter` at the end of `boot.rb`. After editing paths, save the file and playtest. Config fields: [Runtime API](/v0.1.0/essential/runtime-api#configuration).

Next: [Catalog format](/v0.1.0/essential/catalog-format), [Pokemon Essentials](/v0.1.0/integration/pokemon-essentials) or [Essentials migration](/v0.1.0/integration/pokemon-essentials-migration).
