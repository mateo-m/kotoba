...
In a fangame, create `Locales/<locale>.json` beside `Game.exe` ([Installing in a game §5](/v0.1.0/essential/installation#5-your-own-translations)). In a git clone, the same paths are relative to the repo root ([Quick Start](/v0.1.0/essential/quick-start)).
...
`source_text` mappings are for [Essentials migration](/v0.1.0/integration/pokemon-essentials-migration), not greenfield keys.
...
Translators must preserve these. See [Placeholders](/v0.1.0/translators/placeholders).
