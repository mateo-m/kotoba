...
Install: [Installing in a game](/v0.1.0/essential/installation). Sample catalog: `kotoba/samples/bare_rgss/en.json`. Covers layout, boot, and `kotoba:` inline markers after Kotoba loads.
...
Default for release ZIPs: edit `kotoba/boot.rb` on disk and `load "kotoba/boot.rb"` from Script Editor ([Installing in a game](/v0.1.0/essential/installation)). The ZIP’s `boot.rb` already configures catalogs and calls `Kotoba.use_adapter("bare_rgss", …)` when you use the bare adapter package.
...
See [Validation CLI](/v0.1.0/tooling/validation-cli):
