...
class CoreTest < KotobaTestCase
  ...
  def test_loads_hash_catalogs_and_translates_nested_keys
    ...
    assert_equal("A wild Pikachu appeared!", Kotoba.t("battle.wild_appeared", {"pokemon" => "Pikachu"}))
  end
  ...
  def test_namespace_helper_prefixes_keys
    load_sample_catalogs
    battle = Kotoba.namespace("battle")
  ...
  def test_resolves_automatic_language_and_default_fallbacks
    ...
    assert_equal(["fr-CA", "fr", "en"], Kotoba.fallback_chain("fr_CA"))
    assert_equal("Sauvegarder", Kotoba.t("menu.save", nil, {"locale" => "fr-CA"}))
    assert_equal("Quit", Kotoba.t("menu.quit", nil, {"locale" => "fr-CA"}))
  end
  ...
  def test_uses_explicit_fallback_overrides
    ...
    assert_equal(["pt-BR", "en"], Kotoba.fallback_chain("pt_BR"))
  end

  def test_default_option_is_evaluated
    assert_equal("Hello, Oak!", Kotoba.t("missing.key", {"name" => "Oak"}, {"default" => "Hello, {name}!"}))
  end
  ...
  def test_source_text_lookup_accepts_sentences_with_dots
    ...
    assert_equal("legacy.line_0001", Kotoba.source_text_key("Hello. {1}"))
  end
  ...
  def test_strict_missing_translation_raises
    ...
    assert_raise(Kotoba::MissingTranslationError) do
      Kotoba.t("missing.key")
    end
  end

  def test_loads_json_catalogs_directly
    Kotoba.load_json("en", %({"menu":{"pokemon":"Pokemon"}}))

    assert_equal("Pokemon", Kotoba.t("menu.pokemon"))
  end

  def test_rejects_non_string_catalog_leaves
    assert_raise(Kotoba::CatalogError) do
      Kotoba.load_hash("en", {"menu" => {"pokemon" => 25}})
    end
  end
  ...
  def test_plural_messages_work_through_core_lookup
    ...
    assert_equal("2 items", Kotoba.t("battle.item_count", {"count" => 2}))
  end
  ...
  def test_discovers_locale_catalog_files_from_configured_roots
    ...
    assert_equal("Save", Kotoba.t("menu.save"))
    assert_equal("Fight", Kotoba.t("battle.start"))
  ensure
  ...
  def test_locale_change_handlers_fire_when_locale_changes
    ...
    Kotoba.on_locale_change(lambda { |old_locale, new_locale| events << ["registered", old_locale, new_locale] })
  ...
  def test_diagnostics_logs_missing_translations
    ...
    assert_equal("translation missing: missing.key", Kotoba.t("missing.key"))
  ...
  def test_duplicate_key_policy_can_raise_on_merge
    ...
    assert_raise(Kotoba::CatalogError) do
      Kotoba.load_hash("en", {"menu" => {"save" => "Store"}})
    end
  end
  ...
  def test_duplicate_key_policy_warns_when_overriding
    ...
    assert_equal("Store", Kotoba.t("menu.save"))
